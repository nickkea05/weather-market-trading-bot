"""
PostgreSQL version of test_csv_stream.py - XRP
...
def get_db_connection():
    """Get PostgreSQL connection from Railway XRP_DATABASE_URL"""
    database_url = os.getenv("XRP_DATABASE_URL")
    if not database_url:
        error_msg = """
ERROR: XRP_DATABASE_URL environment variable not set

To fix this in Railway:
1. Go to your XRP PostgreSQL database service
...
8. Add a new variable: XRP_DATABASE_URL = (paste the connection string)
...
- Railway will automatically add DATABASE_URL (but you need to rename it to XRP_DATABASE_URL)
"""
...
def fetch_trader_trades(wallet_address: str, market_slug: str, market_title: str, start_time: datetime, end_time: datetime) -> list:
    ...
    try:
        ...
        # Filter for this specific XRP market and time window
        filtered = []
        for t in all_trades:
            title = t.get('title', '')
            
            if 'XRP Up or Down' not in title:
                continue
...
def discover_market() -> dict | None:
    ...
    print("\n[DISCOVERY] Looking for active XRP market...")
    
    for offset in [0, 1]:
        timestamp = get_interval_timestamp(offset)
        slug = f"xrp-updown-15m-{timestamp}"
...
class PostgresBot:
    ...
    def run(self):
        print("=" * 60)
        print("  POSTGRESQL DATA COLLECTOR + TRADER TRACKER - XRP")
